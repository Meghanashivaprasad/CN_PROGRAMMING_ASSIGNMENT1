...
struct _REJECT_PACKET get_reject_packet(struct _DATA_PACKET data_packet) {
    ...
    reject_packet.recieved_segment_number = data_packet.segment_number;
...
struct _ACK_PACKET get_acknowledgement_packet(struct _DATA_PACKET data_packet) {
    ...
    ack_packet.recieved_segment_number = data_packet.segment_number;
...
int main(int argc, char* argv[])
{
    ...
    while(1){
        ...
        segment_count[data_packet.segment_number]++;
        
        // if segment_number is 12 or 11then set segment_count to 1
        // so that if packet repeats it doesn't count fo duplicate packet
        if(data_packet.segment_number == 12 || data_packet.segment_number == 11) {
            segment_count[data_packet.segment_number] = 1;
        }

        if(data_packet.segment_number != expected_packet && data_packet.segment_number != 2) {
        ...
        else if(segment_count[data_packet.segment_number] != 1) {
            reject_packet = get_reject_packet(data_packet);
